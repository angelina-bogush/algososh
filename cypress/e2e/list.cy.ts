...
describe("Linked list", () => {
  ...
  it("should render default list correctly", () => {
    const circles = cy.get('[data-cy="circleWrapper"]').should("have.length", 5);
  ...
  it("should delete from head correctly", () => {
    ...
    cy.wait(DELAY_IN_MS);
    cy.get('[data-cy="circleWrapper"]').first().contains("head");
    cy.get('[data-cy="circleWrapper"]').should("have.length", 4);
  });
  ...
  it("should delete from tail correctly", () => {
    ...
    cy.wait(DELAY_IN_MS);
    cy.get('[data-cy="circleWrapper"]').last().contains("tail");
    cy.get('[data-cy="circleWrapper"]').last().contains(array[array.length - 2]);
    cy.get('[data-cy="circleWrapper"]').should("have.length", 4);
  });

  it("should remove by index correctly", () => {
    // cy.get("input").first().type(array[0].toString());
    cy.get("input").last().type(array[0].toString());
    cy.contains("button", "Удалить по индексу").click();
    cy.get('[data-cy="circleWrapper"]').eq(0).find(circleChangingStyle);
    cy.get(circleChangingStyle).contains(array[0]);
    cy.get('[data-cy="circleWrapper"]').should("have.length", 4);
  });
})